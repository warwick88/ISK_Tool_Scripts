...
--10/20/2020   Sekumar       What: Modifed the code by splitting and documentcodes and adding into temp table, changed the data type for this variale (@DocumentCodeIdAsSystemConfigKey) int to VARCHAR(MAX)  
--							 Why:KCMHSAS Build Cycle Tasks #100
--11/03/2020   Sekumar       What: Added new column(DeferralReason) in temp table(@CustomDocumentNeeds) for iniialize the field(DeferralReason)
--							 Why:KCMHSAS Build Cycle Tasks #105
-- =============================================          
BEGIN TRY        
 BEGIN        
  ...
  DECLARE @DocumentCodeIdAsSystemConfigKey  VARCHAR(MAX)  
  ...
  DECLARE @DocumentCodes TABLE(DocumentCodeID INT)
  INSERT INTO @DocumentCodes(DocumentCodeID)  -- ISP Document
  SELECT @DocumentCodeID
  
  INSERT INTO @DocumentCodes(DocumentCodeID)  -- Assessment and MH Assessment Document
  SELECT ITEM from [dbo].fnSplit(@DocumentCodeIdAsSystemConfigKey, ',')  
...
IN (select DocumentCodeID from @DocumentCodes)    AND ISNULL(RecordDeleted, 'N') <> 'Y'   ORDER BY EffectiveDate DESC    ,ModifiedDate DESC  
