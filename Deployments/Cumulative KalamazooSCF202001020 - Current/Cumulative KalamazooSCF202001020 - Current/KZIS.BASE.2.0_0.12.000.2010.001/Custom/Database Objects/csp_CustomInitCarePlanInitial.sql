IF EXISTS (
	...
	DROP PROCEDURE [dbo].[csp_CustomInitCarePlanInitial] --128172,550,NULL
GO
...
BEGIN
	BEGIN TRY
		...
		DECLARE @LatestsignedPreferenceDocumentVersionId INT
		...
		-- For Preference Latest Documentversionid
		SELECT TOP 1 @LatestsignedPreferenceDocumentVersionId = dv.DocumentVersionId		
		FROM DocumentVersions dv
		INNER JOIN Documents d ON dv.documentid = d.DocumentId
		INNER JOIN DocumentCodes dc ON d.DocumentCodeId = dc.DocumentCodeId
		WHERE d.ClientId = @ClientID
			...
			AND dc.DocumentCodeId IN (
				@CarePlanDocumentcodeId
				,@MHAssessmentDocumentcodeId,60064
				)
			AND ISNULL(d.RecordDeleted, 'N') <> 'Y'
			AND ISNULL(dv.RecordDeleted, 'N') <> 'Y'
			AND ISNULL(dc.RecordDeleted, 'N') <> 'Y'
		...
		Declare @ClientCurrentEpisodeNumber int
		set @CurrentEpisodeNumber = (SELECT MAX(EpisodeNumber)
                                             FROM   dbo.ClientEpisodes
                                             WHERE  ISNULL(RecordDeleted, 'N') = 'N'
                                                    AND ClientId = @ClientId)
		set @ClientCurrentEpisodeNumber = (SELECT MAX(CurrentEpisodeNumber)
                                             FROM   dbo.Clients
                                             WHERE  ISNULL(RecordDeleted, 'N') = 'N'
                                                    AND ClientId = @ClientId)
			IF (ISNULL(@ClientCurrentEpisodeNumber, 0) > 0)	
			BEGIN
			...
			SELECT  @EpisodeRegistrationDate = ce.RegistrationDate
					FROM Clients c
					INNER JOIN ClientEpisodes ce ON ce.ClientId = c.ClientId
						AND ce.EpisodeNumber = @CurrentEpisodeNumber
					WHERE ISNULL(ce.RecordDeleted, 'N') = 'N'
					AND ISNULL(c.RecordDeleted, 'N') = 'N'
						AND c.ClientId = @ClientId
			END
		...
		SELECT TOP 1 @Barriers = Barriers
			,@Abilities = Abilities
		...
		--		,'RE'
		--		)AND
				 ISNULL(DCP.RecordDeleted, 'N') = 'N'
			AND Doc.ClientId = @ClientID
			--DocumentCodeId IN (67005)      
			AND ISNULL(Doc.RecordDeleted, 'N') = 'N'
			AND Doc.STATUS = 22
		ORDER BY dcp.documentversionid DESC

		IF EXISTS (
				SELECT TOP 1 *
				FROM DocumentCarePlans
				WHERE Documentversionid = @LatestsignedDocumentVersionId
				)
			SET @Strengths = (
					SELECT Strengths
					FROM DocumentCarePlans
		...
		PRINT @LatestsignedDocumentVersionId
		...
		BEGIN
			IF (
					...
					AND (
						@EffectiveDate >= @EpsiodeRegistrationDate
						OR @EffectiveDateMHA >= @EpsiodeRegistrationDate
						)
					)
				SET @SameEpisode = 'Y'
		END
		...
		BEGIN
			IF (
					...
					AND (
						@EffectiveDate <= @EpsiodeDischargeDate
						OR @EffectiveDateMHA >= @EpsiodeRegistrationDate
						)
					)
				SET @SameEpisode = 'Y'
		...
		IF EXISTS (
				...
				WHERE CDMA.DocumentVersionId = @LatestsignedPreferenceDocumentVersionId
				)
		BEGIN
			SET @Preferences = (
		...
		BEGIN
			SET @Preferences = (
					...
					WHERE DCA.DocumentVersionId = @LatestsignedPreferenceDocumentVersionId
					)
		END
		...
		CREATE TABLE #CustomTempAuthorizationCodes (
			AuthorizationCodeId INT
			,AuthorizationCodeName VARCHAR(100)
			,CarePlanPrescribedServiceId INT
			,DocumentVersionId INT
			...
			,Detail VARCHAR(MAX)
			,TotalUnits decimal
			,IsInitializedFrom INT
			,TableName VARCHAR(100)
			)

		EXEC csp_GetCustomCarePlanPrescribedServicesAuthorizationCodes @ClientID
			,@LatestCarePlanDocumentVersionID
			,'I';

		IF EXISTS (SELECT 1
				FROM #CustomTempAuthorizationCodes 
				)
		BEGIN
			SELECT AuthorizationCodeId
				,AuthorizationCodeName
		...
		-- PreviouslyRequested              
		EXEC ssp_SCGetPreviouslyRequestedUMCodeUnits @ClientID,@LatestCarePlanDocumentVersionID   
