...
/* Author: Ankita Sinha            */    
/* Modifications               
--01/23/2020   Arul Sonia       What: Removed validation for "Individualized Service Plan Missing Provider: Integrated Services of Kalamazoo"
--								Why:KCMHSAS - Support #1614   
--02/18/2021   Dasari Sunil     What: Need Validation adjusted in QA and PROD for ISP
--								Why:we do complete a treatment plan the same day as we open an episode.  So all we need is the greater than condition changed to greater than or equal to.
								KCMHSAS - Support #1669                                         */    
/* ---------------------------------------------------------------------*/    
...
/*      01-09-2015 - Ajay K.Bangar - Task#639:Upon signing the Care Plan check to see if the Care Plan 'Effective Date' is after the 'Registraion Date' of the Episode.   */    
        ...
        from    DocumentCarePlans    
        where   DocumentVersionId = @DocumentVersionId    
    and cast(@RegistrationDate as date) > cast(@EffectiveDate as date)  
	and   cast(@RegistrationDate as date) <> cast(@EffectiveDate as date)
