...
/****** Object:  StoredProcedure [dbo].[csp_SCPostSignatureUpdateTreatmentPlan]    Script Date: 12/28/2017 17:27:54 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_SCPostSignatureUpdateTreatmentPlan]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_SCPostSignatureUpdateTreatmentPlan]
GO
...
CREATE procedure [dbo].[csp_SCPostSignatureUpdateTreatmentPlan]  
@ScreenKeyId int,                            
@StaffId int,                            
...
select @PlanOrAddendum = tp.PlanOrAddendum         
  from TPGeneral tp        
...
select 'TPGeneral', 'DeletedBy', 'Cannot change from ' + aca.DisplayAs + ' to ' + act.DisplayAs +          
 ...
 where tpp.DocumentVersionId = @DocumentVersionId   
   ...
   AND ( ( @PlanOrAddendum = 'A' AND OriginalTPProcedureId IS NULL ) OR @PlanOrAddendum = 'T' )           
...
insert into @ValidationErrors (TableName, ColumnName, ErrorMessage)        
select 'TPGeneral', 'DeletedBy', 'Cannot modify ' +         
...
insert into @ValidationErrors (TableName, ColumnName, ErrorMessage)        
select 'TPGeneral', 'DeletedBy', 'Cannot decrease Units below Used Units for ' + ac.DisplayAs +         
       '.  Authorization #' + isnull(a.AuthorizationNumber, 'Unknown') + ' has been already in use.'        
  from TPProcedures tpp        
       join Authorizations a on a.AuthorizationId = tpp.AuthorizationId        
       join AuthorizationCodes ac on ac.AuthorizationCodeId = tpp.AuthorizationCodeId        
...
insert into @ValidationErrors (TableName, ColumnName, ErrorMessage)        
select 'TPGeneral', 'DeletedBy', 'Cannot delete ' + ac.DisplayAs + '.  Authorization #' + isnull(a.AuthorizationNumber, 'Unknown') + ' has been already in use.'        
  from TPProcedures tpp        
       join Authorizations a on a.AuthorizationId = tpp.AuthorizationId        
       join AuthorizationCodes ac on ac.AuthorizationCodeId = tpp.AuthorizationCodeId        
 where tpp.DocumentVersionId = @DocumentVersionId    
 ...
 */                                    
 ...
 order by cch.StartDate, cch.COBOrder   
  ...
  begin  
   insert into @ValidationErrors (TableName, ColumnName, ErrorMessage)   
   Select 'TPGeneral', 'DeletedBy','Client does not have a coverage plan that uses authorizations. Please contact administrator'  
  end  
...
RequesterComment = tpg.AuthorizationRequestorComment,         
   ...
   where ad.AuthorizationDocumentId = @AuthorizationDocumentId                                     
     and (isnull(tpg.Assigned, 0) <> isnull(ad.Assigned, 0) or        
...
--raiserror 50010 'Failed to execute csp_SCPostSignatureUpdateTreatmentPlan.'        
