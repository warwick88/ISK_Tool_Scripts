...
--                               Why: There is no use to point CM DB As we are using single DB for Both CM AND Sc DB.   
 ...
 29 JAN 2021  Sunil D    What: Added logic to write the entries into the authorizations table on selecting default options "Integrated Services of Kalamazoo" & "Community/Natural Support" on the Interventions tab.
						 Why: Fails to recognize Direct Ops Providers Recently Added as actual Providers. Not applying logic to create Authorizations and other processes. KCMHSAS - Support #1649
****************************************************************************/           
as                
...
select @ClientId = d.ClientId,  @DocumentCodeId = d.DocumentCodeId,  -- added by shikha    
 ...
 IF @EnableAutoAuthorizationCreation='No'
  ...
  IF EXISTS(SELECT 1 FROM CustomCarePlanPrescribedServices WHERE DocumentVersionId=@DocumentVersionId AND AuthorizationCodeId IS NOT NULL
			  AND FromDate IS NOT NULL AND ToDate IS NOT NULL AND Units IS NOT NULL AND Frequency IS NOT NULL 
			AND TotalUnits IS NOT NULL AND ISNULL(RecordDeleted, 'N') = 'N' AND IsInitializedFrom IS NULL)
	BEGIN
		...
		SELECT CPS.DocumentVersionId,CPS.AuthorizationCodeId,CPS.Units,CPS.Frequency,S.ProviderId,
		case when CPS.ProviderId in (-1,-2) then null 
		else CPS.ProviderId
		end
		,CPS.FromDate,CPS.ToDate,CPS.TotalUnits
		FROM CustomCarePlanPrescribedServices CPS
		left join Sites S on CPS.ProviderId=S.SiteId
		WHERE CPS.DocumentVersionId=@DocumentVersionId AND CPS.AuthorizationCodeId IS NOT NULL
			AND CPS.FromDate IS NOT NULL AND CPS.ToDate IS NOT NULL AND CPS.Units IS NOT NULL AND CPS.Frequency IS NOT NULL 
			AND CPS.TotalUnits IS NOT NULL AND ISNULL(CPS.RecordDeleted, 'N') = 'N' AND CPS.IsInitializedFrom IS NULL
