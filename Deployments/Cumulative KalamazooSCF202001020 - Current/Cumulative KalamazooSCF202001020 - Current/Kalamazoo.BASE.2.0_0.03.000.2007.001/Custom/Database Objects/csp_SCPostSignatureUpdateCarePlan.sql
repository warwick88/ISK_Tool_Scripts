...
select @ClientId = d.ClientId,  @DocumentCodeId = d.DocumentCodeId,  -- added by shikha    
 ...
 IF @EnableAutoAuthorizationCreation='No'
  ...
  IF EXISTS(SELECT 1 FROM CustomCarePlanPrescribedServices WHERE DocumentVersionId=@DocumentVersionId AND AuthorizationCodeId IS NOT NULL
			...
			AND TotalUnits IS NOT NULL AND ISNULL(RecordDeleted, 'N') = 'N')
	BEGIN
		INSERT INTO TPProcedures(DocumentVersionId,AuthorizationCodeId,Units,FrequencyType,SiteId,StartDate,EndDate,TotalUnits)
		SELECT DocumentVersionId,AuthorizationCodeId,Units,Frequency,ProviderId,FromDate,ToDate,TotalUnits
		FROM CustomCarePlanPrescribedServices WHERE DocumentVersionId=@DocumentVersionId AND AuthorizationCodeId IS NOT NULL
			AND FromDate IS NOT NULL AND ToDate IS NOT NULL AND Units IS NOT NULL AND Frequency IS NOT NULL 
			AND TotalUnits IS NOT NULL AND ISNULL(RecordDeleted, 'N') = 'N'
