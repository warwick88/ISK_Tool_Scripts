...
IF EXISTS (
	...
	DROP PROCEDURE [dbo].[csp_InitCarePlanNeeds] 
GO
...
BEGIN TRY      
 BEGIN      
  ...
  SELECT TOP 1 @LatestDocumentVersionIDAsSystemConfigKey = ISNULL(CurrentDocumentVersionId, 0)
			FROM Documents
		WHERE ClientId = @ClientID
			AND CONVERT(DATE, EffectiveDate) <= CONVERT(DATE, GETDATE(), 101)			
			AND STATUS = 22
			AND DocumentCodeId IN (@DocumentCodeIdAsSystemConfigKey)
